...
contract MamiStakeV2 is Ownable, ReentrancyGuard {
    ...
    function claim(
        ...
        if (rewardsAmount > 0) {
            uint256 fee = rewardsAmount / 20;

            address refer = refers[msg.sender];
            if (refer == address(0)) {
                RewardsToken(pool.rewardsTokenAddress).mint(foundation, fee);
            } else {
                uint256 referAmount = (fee * referRatio) / 100;

                RewardsToken(pool.rewardsTokenAddress).mint(refer, referAmount);
    ...
    function setReferRatio(uint256 _referRatio) external onlyOwner {
        require(
            _referRatio >= 0 && _referRatio <= 100,
            "_referRatio must normal value"
        );
